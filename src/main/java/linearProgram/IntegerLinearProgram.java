...
public class IntegerLinearProgram {
	...
	IloCplex cplex;
	private IloIntVar[] X_p;
	private IloIntVar[] X_q;
	private IloIntVar[] X_s;
	...
	public IntegerLinearProgram(String filenameTopology,String filinameSeedsetA,int budget) {		
		this.budget = budget;
...
//			createConstraintHack();
	...
	private void createConstraintHack() {
		int[] ans = {0, 192, 137, 329, 3980, 19, 214, 23, 88, 24, 27, 285, 31, 4000, 32, 34, 36, 166, 230, 617, 42, 173, 111, 4017, 307, 698, 4027, 124};
		try {

			for(int i=0;i<ans.length;i++){
				IloLinearIntExpr exprHack=cplex.linearIntExpr();
				exprHack.addTerm(1,X_s[ans[i]]);
				cplex.addEq(exprHack, 1.0, ": Contraint hack for node " + i);
	...
	public void createObjective() {
		try {
			...
			while (it.hasNext()) {
				...
				expr.addTerm(pair.getValue().intValue(), X_p[(int) pair.getKey()]);
			...
			for (int i =  0; i < NODE_COUNT; i++) {
				for(int j =0; j<NODE_COUNT;j++)
				{
					if(adjacencyMatrix[i][j]==true) //edge (i,j) exists
					{
						...
						expr1.addTerm(1,X_p[i]);
						expr1.addTerm(1, X_q[j]);
						cplex.addLe(expr1, 1+epsilon, ": Contraint 1 for edge (" + i + "," + j +")" );
	...
	public void createConstraint2() {
		try {
			for (int i =  0; i < NODE_COUNT; i++) {
				...
				expr2.addTerm(1,X_p[i]);
				expr2.addTerm(1, X_q[i]);
				expr2.addTerm(1, X_s[i]);
				cplex.addLe(expr2, 1+epsilon, ": Contraint 2 for node" + i );
				cplex.addGe(expr2, 1, ": Contraint 2prime for node" + i );
	...
	public void createConstraint3() {
		try {
			...
			while (iterator.hasNext()){
				IloLinearIntExpr expr3=cplex.linearIntExpr();
				int nodeID = (Integer)iterator.next();
				expr3.addTerm(1,X_p[nodeID]);
				cplex.addLe(expr3, 1+epsilon, ": Contraint 3 for node " + nodeID);
				cplex.addGe(expr3, 1, ": Contraint 3 for node " + nodeID);
				//			   System.out.println("Value: "+iterator.next() + " ");  
			}
	...
	public void createConstraint4() {
		...
		try {
			IloLinearIntExpr expr4=cplex.linearIntExpr();
			for (int i =  0; i < NODE_COUNT; i++) {
				expr4.addTerm(nodeCost.get(i).intValue(),X_s[i]);
			}
			cplex.addLe(expr4, budget+epsilon, ": Contraint 4 " );
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}		
	}
	...
	public void createX_pVariables() {		
		try {
			X_p = cplex.intVarArray(NODE_COUNT, 0, 1);
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}	
	...
	public void createX_qVariables() {		
		try {
			...
			X_q = cplex.intVarArray(NODE_COUNT, 0, 1);
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}	
	...
	public void createX_sVariables() {		
		try {
			...
			X_s = cplex.intVarArray(NODE_COUNT, 0, 1);
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}	
	...
	public void printValue()
	{
		try {
			...
			for(int i=0;i<NODE_COUNT;i++)
			{
				...
				expr.addTerm(nodeUtility.get(i).intValue(), X_p[i]);
			}
			System.out.println("\n obj value ="+cplex.getValue(expr));
			//		System.out.println(cplex.toString());
			try 
			{
				PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("ILPoutputtfile.txt", true)));
				...
				outobj.println("ILP obj value ="+cplex.getValue(expr));
				outobj.close();
				if(DEBUG)
					System.out.print("\n");
				out.print("\n");
